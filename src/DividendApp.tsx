...
const DividendApp: React.FC = () => {
  ...
  const searchStock = async (query: string) => {
    ...
    try {
      ...
      // 1단계: Yahoo Finance API 시도 (무료, 실시간)
      try {
        ...
        if (yahooResponse.ok) {
          ...
          if (yahooData.quotes && yahooData.quotes.length > 0) {
            results = yahooData.quotes.slice(0, 10).map((quote: any) => ({
              symbol: quote.symbol,
              name: quote.longname || quote.shortname || quote.symbol,
              price: quote.regularMarketPrice || (Math.random() * 200 + 20).toFixed(2),
            ...
            console.log(`Yahoo Finance 성공: ${results.length}개 결과`);
          }
        }
      } catch (yahooError) {
        console.log('Yahoo Finance API 실패:', yahooError);
      }
      
      // 2단계: Yahoo가 실패하거나 결과가 없으면 Finnhub API 시도
      if (results.length === 0) {
        try {
          console.log('Finnhub API 호출 중...');
          const finnhubUrl = `https://finnhub.io/api/v1/search?q=${encodeURIComponent(query)}&token=demo`;
          const finnhubResponse = await fetch(finnhubUrl);
          
          if (finnhubResponse.ok) {
            const finnhubData = await finnhubResponse.json();
            if (finnhubData.result && finnhubData.result.length > 0) {
              results = finnhubData.result.slice(0, 10).map((item: any) => ({
                symbol: item.symbol,
                name: item.description,
                price: (Math.random() * 200 + 20).toFixed(2),
                dividendYield: (Math.random() * 8).toFixed(2),
                exchange: 'US',
                quoteType: item.type || 'EQUITY',
                isEtf: item.type === 'ETF' || item.description?.includes('ETF')
              }));
              console.log(`Finnhub 성공: ${results.length}개 결과`);
            }
          }
        } catch (finnhubError) {
          console.log('Finnhub API 실패:', finnhubError);
        }
      }
      
      // 3단계: 모든 API가 실패하면 확장된 로컬 데이터 사용
      if (results.length === 0) {
        console.log('모든 API 실패, 로컬 데이터 사용');
        // API 한도 초과 또는 검색 결과 없을 때 로컬 데이터 사용
        const localStocks = [
          // 주요 개별 주식
          ...
          { symbol: 'JNJ', name: 'Johnson & Johnson', price: 160.80, dividendYield: 2.98 },
          { symbol: 'PG', name: 'Procter & Gamble Co.', price: 152.30, dividendYield: 2.45 },
          { symbol: 'PFE', name: 'Pfizer Inc.', price: 28.90, dividendYield: 5.85 },
          { symbol: 'T', name: 'AT&T Inc.', price: 15.25, dividendYield: 7.20 },
          { symbol: 'VZ', name: 'Verizon Communications Inc.', price: 38.50, dividendYield: 6.95 },
          ...
          { symbol: 'XYLD', name: 'Global X S&P 500 Covered Call ETF', price: 45.20, dividendYield: 10.15 },
          { symbol: 'RYLD', name: 'Global X Russell 2000 Covered Call ETF', price: 14.85, dividendYield: 11.25 },
          { symbol: 'DIVO', name: 'Amplify CWP Enhanced Dividend Income ETF', price: 37.90, dividendYield: 5.12 },
          { symbol: 'NUSI', name: 'Nationwide Risk-Managed Income ETF', price: 19.75, dividendYield: 7.35 },
          ...
          { symbol: 'HDV', name: 'iShares Core High Dividend ETF', price: 108.60, dividendYield: 3.15 },
          { symbol: 'DGRO', name: 'iShares Core Dividend Growth ETF', price: 52.40, dividendYield: 2.08 },
          { symbol: 'NOBL', name: 'ProShares S&P 500 Dividend Aristocrats ETF', price: 95.30, dividendYield: 1.85 },
          { symbol: 'DVY', name: 'iShares Select Dividend ETF', price: 125.80, dividendYield: 3.25 },
          
          // 부동산 리츠 ETF
          { symbol: 'VNQ', name: 'Vanguard Real Estate Index Fund ETF', price: 87.20, dividendYield: 3.68 },
          { symbol: 'SRET', name: 'Global X SuperDividend REIT ETF', price: 8.45, dividendYield: 8.92 },
          { symbol: 'MORT', name: 'VanEck Mortgage REIT Income ETF', price: 16.30, dividendYield: 9.15 },
          
          // 국제 배당 ETF
          { symbol: 'VXUS', name: 'Vanguard Total International Stock ETF', price: 58.90, dividendYield: 3.12 },
          { symbol: 'VYMI', name: 'Vanguard International High Dividend Yield ETF', price: 63.40, dividendYield: 4.25 },
          { symbol: 'HDV', name: 'iShares High Dividend ETF', price: 104.80, dividendYield: 3.5 },
          { symbol: '005930.KS', name: '삼성전자', price: 71000, dividendYield: 2.1 },
          { symbol: '000660.KS', name: 'SK하이닉스', price: 123000, dividendYield: 1.2 },
          { symbol: '035720.KS', name: '카카오', price: 45500, dividendYield: 0.8 }
        ];
